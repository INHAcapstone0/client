...
function AccountReceiptUploadPage({route, navigation}: any) {
  ...
  const [data, setData] = useState([
    {
      ...
      name: route.params.item.print_content,
      quantity: '1',
      price: route.params.item.tran_amt,
    },
  ]);
  ...
  //사용자가 입력한 결제한 금액
  const [totalPrice, setTotalPrice] = useState(route.params.item.tran_amt);
  ...
  useEffect(() => {
    console.log('route.params.data', route.params.item);
    // setData([
    //   {
    ...
    //   },
    // ]);
    setPlace(route.params.item.branch_name);
    setPayTime(
      new Date(
        route.params.item.tran_date.substring(0, 4),
        route.params.item.tran_date.substring(4, 6),
        route.params.item.tran_date.substring(6, 8),
        route.params.item.tran_time.substring(0, 2),
        route.params.item.tran_time.substring(2, 4),
        route.params.item.tran_time.substring(4, 6),
      ),
    );
    addSelectedDate(
      route.params.item.tran_date.substring(0, 4) +
      ...
      <AlertNotificationRoot
        ...
        <ScrollView>
          ...
          <Calendar
            ...
            initialDate={`${route.params.item.tran_date.substring(
              0,
              4,
            )}-${route.params.item.tran_date.substring(
              4,
              6,
            )}-${route.params.item.tran_date.substring(6, 8)}`}
            theme={{
