...
function ReceiptResultPage({route, navigation}: any) {
  ...
  //카테고리 변수
  const [category, setCategory] = useState(route.params.data.store.category);
  ...
  useEffect(() => {
    console.log('route.params.screenShot', route.params.screenShot);
  ...
  const uploadItems = async (receiptId: string) => {
    ...
    try {
      ...
      console.log('item response', receiptId);

      // console.log('item response', response.data[0].receipt_id);

      uploadImage(receiptId);
    } catch (err: AxiosError | any) {
      console.log(err);
      Toast.show({
        type: ALERT_TYPE.WARNING,
...
//       console.log('response upload', response.data.id);
